...
def optimize_shopping_route(user_id, origin_lat, origin_lng, shopping_list, destination_lat=None, destination_lng=None):
    """
    ...
    request_data = {
        ...
        'destination': {'lat': destination_lat, 'lng': destination_lng} if destination_lat else None,
        'stops': []
    }
    ...
    for item in shopping_list:
        request_data['stops'].append({
            ...
            'seller_name': item['seller'].shop_name or item['seller'].full_name,
...
"""
    ...
    try:
        ...
        for order_idx, stop_idx in enumerate(optimized_order):
            if isinstance(stop_idx, int) and 0 <= stop_idx < len(shopping_list):
        ...
        return {
            ...
            'stops': [shopping_list[i] for i in optimized_order if isinstance(i, int) and 0 <= i < len(shopping_list)]
        }
        
    except Exception as e:
        logger.error(f"Route optimization failed: {e}")
        db.session.rollback()
        return fallback_route_optimization(user_id, origin_lat, origin_lng, shopping_list, destination_lat, destination_lng)

def fallback_route_optimization(user_id, origin_lat, origin_lng, shopping_list, destination_lat=None, destination_lng=None):
    ...
    while remaining:
        nearest = min(remaining, key=lambda x: haversine_distance(
            ...
            x['seller'].shop_latitude, x['seller'].shop_longitude
        ))
...
def get_route_plan(route_plan_id):
    ...
    stops = RoutePlanStop.query.filter_by(route_plan_id=route_plan_id).order_by(
        RoutePlanStop.stop_order
    ).all()
